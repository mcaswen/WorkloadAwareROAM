...
**研究状态：双基线已可运行，阶段级自适应研究处于实验基础设施建设阶段。**

Classic 和 DOD 基线、统一误差口径、固定三角形预算、拓扑验证、增量网格提交、OpenGL/D3D12 双图形后端、无窗口基准测试和应用级运行时基准测试均已可运行。阶段划分、合法策略、研究假设、实验方法和源码改造顺序已经定义；按阶段独立控制策略、冻结状态重放、统一结果哈希、离线最优参考和在线选择策略尚未实现。
...
仓库包含 SDL2、GLM、GLAD、Dear ImGui、stb 和 Windows portable CMake。普通预设会优先使用系统包，再使用项目内 `third_party`；默认不主动联网。
...
| 阶段级策略变体 | 计划在 DOD 内建立的受控实验路径 | 合法实现、请求值与实际值、回退原因和记录字段已经定义 | 尚未完成按阶段控制、冻结状态重放和结果哈希 |
...
| 合并拓扑 | 安全的内部候选可以并行辅助处理；结果合并、活动索引、`Q_m` 更新和最终收敛由主线程完成 | 在相同冻结输入上比较串行立即处理与并行辅助处理 |
| 细分拓扑 | 已有可复用子节点的安全候选可以并行辅助处理；预算、强制细分链和最终收敛由主线程完成 | 显式记录回退原因，并验证最终活动切面等价 |
| 网格提交 | 拓扑编辑重放、槽位和区间维护串行；脏槽位写入超过阈值时可并行 | 增加全量对照，并统一脏槽位、脏区间和结果哈希 |
...
截至 2026-08-22，`RelWithDebInfo` 配置下现有 10 项 CTest 均通过。
...
可选算法为 `classic|dod|all`，可选配置为 `smoke|budget-reentry|budget-saturation|incremental-emit|standard`。其中 `incremental-emit` 使用重复视点区分首次建立、一次属性变化和随后无脏区间复用。
...
- 阶段级策略控制、`PassTrace`、冻结状态重放和统一结果哈希尚未实现
...
1. 增加按阶段记录的 `PassTrace`，区分请求实现、实际实现、线程数量、回退原因、结构特征和完整耗时
2. 建立冻结状态重放、标准化结果和统一哈希，使同一阶段的合法实现能够成对比较
