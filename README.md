...
**研究状态：阶段 0 的观测基线、阶段 1 的显式策略对照和阶段 2 的评分语义拆分已完成。**

Classic 和 DOD 基线、统一误差口径、固定三角形预算、拓扑验证、OpenGL/D3D12 双图形后端、无窗口基准测试和应用级运行时基准测试均已可运行。当前可以独立控制合并评分、细分评分、合并拓扑、细分拓扑、网格提交和 CPU 上传；固定串行/最大安全并行与增量/全量输出组成的四种对照路径已经通过固定轨迹等价性回归。`Q_s/Q_m` 已分别记录全量评分、原地建堆、候选快照和局部成员维护成本。冻结单个阶段输入的配对计时、离线最优参考和在线选择策略尚未实现。
...
| 阶段级策略变体 | DOD 内的受控实验路径 | 已实现固定串行/最大安全并行与增量/全量输出的四种组合及自定义阶段策略；评分、建堆、候选快照和成员维护成本分别记录 | 当前全量网格写入是串行实现；单个阶段的冻结输入配对留到后续阶段 |
...
| 合并拓扑 | 安全的内部候选可以并行辅助处理；结果合并、活动索引、`Q_m` 更新和最终收敛由主线程完成 | 已支持 `SerialImmediate` / `ParallelAssisted`；冻结候选快照配对仍待实现 |
| 细分拓扑 | 已有可复用子节点的安全候选可以并行辅助处理；预算、强制细分链和最终收敛由主线程完成 | 已支持 `SerialImmediate` / `ParallelAssisted`；冻结候选快照配对仍待实现 |
...
截至 2026-08-22，`RelWithDebInfo` 配置下现有 14 项 CTest 均通过。
...
可选算法为 `classic|dod|all`，可选配置为 `smoke|budget-reentry|budget-saturation|incremental-emit|pass-trace-replay|pass-policy-replay|standard`。其中 `incremental-emit` 使用重复视点区分首次建立、一次属性变化和随后无脏区间复用；`pass-trace-replay` 会在重置后重复固定轨迹；`pass-policy-replay` 会依次运行固定串行增量、最大安全并行增量、固定串行全量和最大安全并行全量，并逐帧比较拓扑、活动叶、预算与规范化网格。
...
- 阶段级策略控制已经实现，但单个阶段的冻结输入保存与配对计时尚未实现；当前入口通过重置后确定性重建固定轨迹
...
4. 建立冻结候选快照和拓扑阶段配对，使串行与并行辅助实现能够在同一输入上比较
5. 运行胜负反转实验；若 H1 不成立，立即收缩或更换研究问题
6. 建立贪心阶段参考和选定帧的整帧离线参考，测量阶段之间的耦合
7. 训练并实现低成本在线选择策略，与固定串行、最大安全并行和最佳静态组合比较
8. 在未参与训练的地形、轨迹、线程配置和时间预算上验证泛化与正确性
9. 仅当主研究完成后确认活动索引或持久队列局部维护成为稳定瓶颈，再把 `FrontierMaintenance` 作为可选实现加入；默认保留当前立即增量维护，不让该实验阻塞主线
