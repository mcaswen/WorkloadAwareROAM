...
**研究状态：阶段 0 的观测基线和阶段 1 的显式策略对照已完成，阶段 2 的评分语义拆分尚未开始。**

Classic 和 DOD 基线、统一误差口径、固定三角形预算、拓扑验证、OpenGL/D3D12 双图形后端、无窗口基准测试和应用级运行时基准测试均已可运行。当前可以独立控制合并评分、细分评分、合并拓扑、细分拓扑、网格提交和 CPU 上传；固定串行/最大安全并行与增量/全量输出组成的四种对照路径已经通过固定轨迹等价性回归。冻结单个阶段输入的配对计时、离线最优参考和在线选择策略尚未实现。
...
| 阶段级策略变体 | DOD 内的受控实验路径 | 已实现固定串行/最大安全并行与增量/全量输出的四种组合及自定义阶段策略；记录请求值、实际值、线程数量、规模、回退原因和耗时 | 当前全量网格写入是串行实现；单个阶段的冻结输入配对留到后续阶段 |
...
| --- | --- | --- |
| 合并评分 | `Q_m` 成员局部维护，活动成员的评分整批刷新；评分计算可并行，堆重建串行 | 已支持 `SerialRefresh` / `ParallelRefresh`；下一阶段拆分评分与堆重建计时 |
| 细分扫描与评分 | `Q_s` 成员局部维护，活动成员的评分整批刷新；评分可并行，堆重建串行 | 已支持 `SerialRefresh` / `ParallelRefresh`；候选快照归入拓扑准备 |
...
3. 建立冻结阶段输入和更细的阶段计时，使同一阶段的合法实现能够成对比较
