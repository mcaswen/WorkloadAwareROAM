...
2026-09-17，调查已提交为`febb0d1`。用户随后要求按04实施情况重整大规划，并允许必要时从新策略回退旧策略。现承接到[QPC-06A认证等价降本](../../plans/cpu_refinement/qpc_06a_quality_cost_reduction_plan.md)，取消未实施04H的重复归属；[大规划第2节](../../plans/cpu_refinement/transactional_quality_performance_closure_major_plan.md)明确原排序C、旧质量L、新逐点P及回退边界。

该决定允许准备有限成本修复，不把P当作必须保留的终点。相同输入重复失败归条件06B，Fit与质量目标修订仍归04，预留归05；是否实际进入各小阶段仍按逐阶段验收。当前没有实施降本或切换配置，原性能问题、持续恢复限制与本报告数值保持不变。
