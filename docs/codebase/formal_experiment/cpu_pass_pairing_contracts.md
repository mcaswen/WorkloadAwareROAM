...
> 状态：P5 三次配对及审查已完成，建议收缩到网格提交；环境与泛化限制保留，当前事实见第 14 节
...
## 14. PREP-05 的探索判断与已完成记录

FACT：2026-09-11 用户确认 P5 小规划后，复用同一当前 OpenGL 程序、冻结输入和 113 个目标，完成三次 W=3/R=12/P=8 的严格配对。三个运行共同输入、执行前特征及结果身份一致，共计 9,000 条计量、2,250 条预热及 12,000 条标定。没有修改生产或正式脚本，源码摘要保持 `ba6e33c4...`，没有重跑 CTest、普通性能矩阵或 D3D12 采样。

FACT：P5 的事前契约为 `cpu-pilot-direction-v1`，记录在 `benchmark-output/prep-05/pilot-20260911/protocol.json`，不修改 C++ 配对 v2、测量协议 1 或正式噪声规则 v1。归档 `collect.py` 仅调用现有 `run_cpu_pilot.py`；`summarize.py::strict_outcome` 从已审计比较中严格要求三个不同 RunId 全部同向，拒绝混入平局、反向、回退、缺失或并行不足。现有 `short_run_stability` 与 P4 排除保持，新探索契约不是新增的正式分析器 API。

FACT：三个运行通过既有独立分析，得到 483 条逐运行目标策略比较；严格汇总为 161 个三次比较。合并评分有串行 1 / 并行 17 个稳定点，细分评分为 0 / 19，合并拓扑为 2 / 0 且回退 12 个，细分拓扑全部 24 个回退。网格串行脏写/并行脏写为串行 4 / 并行 3，另有 4 个三次平局和 13 个含平局或不一致，只有该阶段满足本轮双向信号规则。

FACT：`review_evidence.py` 只为全部目标比较补充既有 `pre_` / `planning_` 字段并核查归档，不更改判断。网格串行侧预计脏数量为 6～18、区间数 5～15，并行侧分别为 739～1,646、584～1,295；脏比例范围重叠。全部执行前特征在三个运行中一致，未使用 `post_` 或执行后耗时作为预测变量。

INFERENCE：绝对规模和规划工作量可初步描述本轮网格两侧，单一脏比例不足；串行侧全部为 Test129、并行侧为 Peking 中/高预算，尚不能分离地形与规模影响或推断泛化。本轮 250 个目标/动作中有 243 个超过旧 5% 漂移参考，原值保留，新探索方向不等于正式环境验收。

FACT：P5 规定工作已完成，审查建议收缩到网格提交，当前未满足完整路线至少三个阶段的条件；未自动补测、启动 PREP-06 或改变拓扑策略。最终投入范围仍由用户决定，当前没有新在线策略或预测器。完整来源、逐点证据、成本及规范比对见[阶段审查](../../reviews/formal_experiment/prep_05_cpu_crossover_pilot_gate_review.md)和[P5 实施记录](../../plans/formal_experiment/prep_05_cpu_crossover_pilot_gate_plan.md#9-实施与闭环记录)。
