...
4. 同一版本、相同相关条件下已经完成的验证可复用；改变相关实现或条件后，应重跑受影响部分。计划内的必要正确性门禁和研究独立重复不能省略，也不能把旧版本结果写成本版本实测。
...
2026-09-10 起按用户要求采用以下默认口径，替代“任意差值超过原极差即持续阻断”的工程验收方式。各小规划在改代码前引用本节；对旧报告保留原规则和原结论，不用新门槛改写历史数据。

1. 每个实现阶段仍保存修改前基线和修改后报告，核对同条件的输入、结果、整体及受影响路径。独立进程为主要统计单位，默认一次预热、五次独立计量，版本顺序交替；同时报告每进程中位数/P95、配对差、绝对及相对变化。
2. 普通帧和阶段耗时的默认调查门槛为 `max(0.05 ms, 5% × 修改前基线统计值, 修改前五次统计值极差)`。这里的基线统计值是对应进程级中位数或 P95 的跨进程中位数；恶化以同期 B−A 配对差的中位数判断。以秒记录的耗时先统一单位，内存、吞吐和资源上限使用小规划的独立预算，不套用毫秒门槛。
3. 首轮超过门槛才完整复测一次；两轮均同向超过该冻结门槛，才作为需要处理的工程性能问题。没有稳定复现的项注明条件和波动，不无限追加采样。局部小于 0.05 ms 的变化默认只记录，不要求两批 AA/BB 或微秒因果定位；若其高频累计已使整体耗时跨过门槛，则由整体指标触发调查。
