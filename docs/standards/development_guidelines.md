...
1. 未经用户许可，不得将内容提交到 Git 仓库，包括本地提交（`git commit`）和远程推送（`git push`）。完成开发、修复或阶段验收不代表获得提交许可。用户已明确授权当前任务逐阶段提交时，该授权在任务内持续有效，不需要每个小阶段重复确认；远程推送仍按用户授权范围处理。
...
6. 新增和适配代码以 `src/algorithms/data_oriented_roam/` 为风格参考：分行表达控制流和状态修改，使用清晰命名、显式分支及职责分离。不要模仿当前 Transactional 实现的紧凑排版，也不要在一行堆叠多个赋值、循环或判断；其历史风格调整另行处理。
...
3. 在用户已许可提交的前提下，每完成一个小阶段的必要验证、结果回填和审查，就提交一次，再开始下一阶段。必要时可按可验证子阶段提交；共享依赖确实无法安全拆开时可合并相邻阶段，但必须说明范围和原因，避免多个阶段长期堆在工作区。
