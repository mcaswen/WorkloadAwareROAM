...
固定样本坐标u/v及数值域T，坐标算子C不读取参考第三分量，因此`C(u,v,h)=C(u,v,0)`。实现保留原区间除法及公开域转换顺序，精确域不经过double中间坐标；覆盖与认证结果保持。c次坐标请求中只有h次需要高度时，参考插值从c次变为h次，但随后参考可能重复构造两个坐标，这部分也必须计费。没有改变O(sd)覆盖界或跨帧质量政策。详见[推导、反例和有限验证](../cpu_refinement/qpc_06g_coordinate_evidence_results.md)。

更新：2026-09-17。配套[推导索引与全部 Lean 文件](README.md)。本页将散落推导按模型串成可连续阅读的数学说明；详细证明过程、失败尝试与实验仍以各节原文为准。

### QPC-06A：同证据的等价计算

固定状态/视图、样本和表示域后，参考坐标、高度与参考投影是不可变函数值；保持原表达式顺序的惰性复用不改变区间端点和精确值。有限binary64写为$x=(-1)^s m2^e$，则累计所需整数为$L(x)=\lfloor(-1)^s m2^{e+128}\rfloor$、$U(x)=-L(-x)$，可由整数移位及负余数修正直接求出。逐样本结果及累计整数相同，再保持目录顺序即可维持同批次结果。详见[QCOST-01～04](../cpu_refinement/quality_cost_reduction_derivation.md)。这是降本等价性，不是新的质量保证或恢复证明。

### QPC-06B：不变状态的完整空结果

令$X$为同一状态实例、代次、源/Q、配置、持续状态及诊断模式，$F(X)$为正常完成的确定性逻辑规划结果。若$F(X)=B$且$B$没有事务，则应用不改状态，$X'=X$，故$F(X')=B$。保存完整$B$保留人口、目录、预算及失败结果；实际执行工作不重放。真正视图/拓扑变化前清除，未完成求解不记录，不能用相同网格哈希替代完整输入不变。

结果包含前缀$r$、池$m$和尝试记录$a$，查键$O(1)$，复制/销毁$O(r+m+a)$。连续$z$次不变机会将$(z+1)W_{plan}$降为$W_{plan}+O((z+1)(r+m+a))$；移动首次求解成本不变。详见[IDLE-01～04](../cpu_refinement/idle_plan_reuse_derivation.md)和[有限结果](../cpu_refinement/qpc_06b_idle_plan_reuse_results.md)。这是纸面推导与定向验证，没有新Lean，也不是质量恢复或跨代次复用证明。
...
[QPC-04F](../../plans/cpu_refinement/qpc_04f_target_quality_policy_plan.md)已完成默认关闭的持续接入；请求、接收自身正进展、独立回收证书及双表示域对应见QC-10～13。其依据为纸面推导、代码与有限核查，下面抽象Lean公式的生产证明范围没有扩大。ATT扩展已暂停，不用一般总定理代替这些接入义务。
...
运行时请求采用缓存数值A(t)>E*²；不再与旧复合P资格相交。对于核心binary64与实际float曲面，各自要求a_new(q)≤max(a_old(q),E*²)、u_new(q)≤max(u_old(q),H*²)。接收侧的超标平方误差和严格下降，回收侧非增；共享样本只能有不变贡献，故去重组合仍非增且非空接收具有正进展。

有理数d的求和证据采用L=floor(2^128 d)、U=ceil(2^128 d)，逐项累加仍包住精确有限和；下界为零不能因d>0的直觉自动接受。近门槛缓存请求与精确认证是不同数值契约。

[04F结果](../cpu_refinement/qpc_04f_target_quality_results.md)实际验证六批47交换的两域条件，但Sierra/Canyon仍停滞。该数据直接说明：逐点损伤非增与正进展条件不能推出目录完备、给定帧数恢复或低成本。ATT暂停，未添加一般总定理来替代这些未解决义务。

### QPC-04G：从接受条件到提案可行域

[PF-01～06完整推导与过程](../cpu_refinement/proposal_feasibility_derivation.md)已完成。固定连接、UV插点、旧高度及Q时，重心系数只让新高度z线性进入曲面：`h(q;z)=a_q+b_qz`。a必须从新连接重建，不能假定未拟合曲面严格等于旧曲面。B固定源高中点是单例，不具有可改高自由度。

设参考高度r，齐次投影`m(h)=d+c h`，`w_r=d_w+c_wr>0`。相同参数位置的屏幕差消去交叉项，得到

\[
A_q(h)=K_q\frac{(h-r)^2}{w(h)^2},\quad
K_q=\left[\frac{W(c_xd_w-c_wd_x)}{2w_r}\right]^2+
\left[\frac{H(c_yd_w-c_wd_y)}{2w_r}\right]^2\ge0.
\]

对`T_q=max(A_old(q),E*²)`、K>0、w(h)>0，约束等价于`|h-r|≤s w(h)`，`s=√(T_q/K_q)`。代入h=a+bz，即

\[
(1-sc_w)b z\le s(d_w+c_wa)-(a-r),\qquad
(-1-sc_w)b z\le s(d_w+c_wa)+(a-r).
\]

高度半径`max(|h_old-r|,H*)`也给两条仿射约束，正w与近面另保留。用有理下/上包络`l≤s≤u`分别形成充分内域与必要外域；在w>0域内有`F_inner⊆F_true⊆F_outer`。外包域空证明无解，内域空并不证明无解。K=0时屏幕项恒零，投影域和高度条件仍必须满足。

安全不等于进展。若必要域为单点z0，且完整Q上`a_q+b_qz0=h_old(q)`，所有安全解都重现旧曲面，超额损失和严格下降不可能。若用零系数论证，必须同时要求旧超标样本`b=0且a=h_old`；只有b=0不能排除新连接已改变常数高度。8个自然项获得单点无进展证书，而非仅试值失败。

新逐点安全及总超额下降，不要求所有点都低于“旧补丁最大值−0.01px”。28个自然见证独立核心/float及原生认证成立，但旧统一门槛必要域在固定最大点出现零系数负右端，原生旧Accepts均拒绝。所有28项局部Emax不变，其中12项进展仅约10⁻¹⁵～10⁻¹⁴；正进展没有给出恢复速率。

可发布见证先存binary64，再对float世界曲面独立重建闭Q并认证，不能从核心安全域直接推出输出安全。完整分类及成本见[04G结果](../cpu_refinement/qpc_04g_proposal_feasibility_results.md)。本轮证据是纸面推导、解析夹具、有限精确核查；没有新增Lean声明，也没有修改生产Fit或证明持续恢复。

### QPC-06D：独立根调度与确定结果

[完整过程与源码义务](../cpu_refinement/qpc_06d_receiver_task_balance_results.md#3-从热点到实现什么变了什么成本没有消失)。固定快照S与根前缀i，每根`F_i(S)`只读共享状态、只写第i个输出及当前任务账本。原子索引唯一领取全部i，同步完成后仍得`A[i]=F_i(S)`；后续按原前缀消费A，因而调度完成顺序不改变事务选择。异常先排空，正常配额不触发为同结果论证前提；没有声称不同超时时序必须生成同一失败人口。

几何工作不变，新增领取O(r+p)，账本空间O(p)。理想独立任务列表调度满足：

\[
\max\left(\frac{\sum_i t_i}{p},\max_i t_i\right)\le T_{list}
\le\frac{\sum_i t_i}{p}+\left(1-\frac1p\right)\max_i t_i.
\]

相较固定连续块`max_j Σ_{i∈C_j}t_i`，该界消除任意重块集中造成的额外尾部，但最长单根仍不可分。实际原子、缓存、同步、抢占另有成本，不能把理想式称为机器时间保证。有限Tracy观察支持任务和相近而最长任务约减半，perf仍保留原数值热点；没有证明必要语义span或添加Lean声明。

### QPC-04H：生成器不属于接受定理的必要前提

固定合法目录项，只把唯一新点高度改成

\[
z_s=\operatorname{Eval64}_{bilinear}(H,u_s,v_s).
\]

旧点/连接/参数域接口保持，原形状条件通过，且实际双域完整证书均成立，才能沿用QC的损伤与正进展结论；不能从“取源高度”直接推出安全。源高只是高度可行域中的一个试值，失败不等于可行域为空。

令两生成路径访问项为$V_0,V_s$，旧Fit通过集为$A_0$，源高结构通过集为$A_s$，则

\[
W_0=\sum_{i\in V_0}F_i+\sum_{i\in A_0}Q_i+W_{downstream,0},\qquad
W_s=\sum_{i\in V_s}S_i+\sum_{i\in A_s}Q_i+W_{downstream,s}.
\]

没有$W_s\le W_0$的一般结论：认证人口和持续事务可以增加，$V_0$与$V_s$也可能不同。源高赋值O(1)，局部结构实现O(d log N+d log d)，后续认证仍最坏O(sd)并另计有理位长。详见[SH-01～04完整过程](../cpu_refinement/qpc_04h_source_height_derivation.md)；这是纸面/有限验证，没有增加Lean证明范围。

### QPC-06E：同表达式的面证据复用

[FE-01～03原文与被拒绝变换](../cpu_refinement/qpc_06e_face_evidence_results.md#3-从函数热点到等价变换推导过程)。冻结面a,b,c与样本q时，边谓词的`b−a`、插值分母`A=(b_u−a_u)(c_v−a_v)−(b_v−a_v)(c_u−a_u)`都与q无关。缓存这些原表达式子式，保留w0/w1/w2及最终高度加法树，按节点代入可得原区间端点/有理值。不能以实数代数等价为由改成反向边取负或乘倒数，舍入与跨零行为可能不同。

相同边值加相同面/边顺序推出首个闭覆盖面相同；相同高度和原样本证据推出相同损伤/进展判断。核心与输出float分别准备，逻辑访问配额不减少，正常未超时运行中事务选择和输出保持。纸面论证加840组数值oracle和有限自然轨迹，不新增Lean或连续曲面保证。

设边查询e、高度查询h、准备面数b。仅计固定坐标减法：

\[
C_{old}=2e+6h,\qquad C_{prepared}=8b.
\]

该式分别适用于区间与有理域；不是全部运算/CPU时间。有理位长和样本相关计算仍需付费。面系数O(d)工作与空间，总覆盖仍最坏O(ud)。早拒绝只查一边时，2次固定减法可能变8次，因此没有逐提案必然提速定理。有限实测支持当前轨迹完整收益，不证明旧Fit、视图或持续状态维护已经接近必要成本。

### QPC-06F：充分拒绝、提示重证与收支

[HR-01～05及反例完整过程](../cpu_refinement/qpc_06f_height_rejection_derivation.md)。对当前样本的旧/新高度残差平方u0、u1和高度目标平方c，保守区间分别为U0、U1、C。有限界下：

\[
\underline U_1>\max(\overline U_0,\overline C)
\Longrightarrow
u_1\ge\underline U_1>\max(\overline U_0,\overline C)\ge\max(u_0,c).
\]

因此当前点已违反必要安全条件，原完整证书不能接受。等号、重叠或非有限只是不足以拒绝，不能改为epsilon判断。S1保留当前人口检查、原高度表达式，只将该严格拒绝前移到屏幕求值前。

S2只缓存样本身份q。当前支持成员、当前闭覆盖、参考人口和高度重证齐备后才能应用上述蕴含；提示不拒绝则从原首样本重新完整认证。故过期提示可能费时，却不能借用旧结论拒绝，也不能产生成功证书。正常未触限前提下，接受集合相同加固定目录/根序及原预算/冲突规则，归纳得到相同实际事务和状态。首个诊断拒绝理由不必相同，配额退出不在正常等价范围内。

以S1为基准，完整支持准备c_i、原数值遍历v_i、提示成本h_i、未拒绝时剩余遍历v'_i、实际早拒绝集合A满足：

\[
W_1=\sum_i(c_i+v_i),\qquad
W_2=\sum_i(c_i+h_i)+\sum_{i\notin A}v'_i+W_{merge};\qquad
W_2<W_1\iff\sum_{i\in A}v_i+\sum_{i\notin A}(v_i-v'_i)>\sum_i h_i+W_{merge}.
\]

提示未拒绝时，同次面证据可能已被初始化，不能机械写v'_i=v_i；固定非负原语模型下v'_i≤v_i只使“被省遍历大于提示费”成为充分条件。完整O(s log s)支持整理与未命中最坏O(sd)仍在；固定C=2048仅约束提示空间/查找，不使认证成为O(1)。失败点很早或无复遇就是不划算的反例。有限Sierra34,705次提示拒绝全部原认证确认；同政策Sierra/Canyon移动完整分别下降34.8%/38.5%。这是纸面、有限数值/轨迹和性能证据，无新增Lean，不证明持续恢复、DOD竞争力或优化空间耗尽。
