...
阶段进度：CBI-01 已完成来源冻结、核心测试与四容量编译门禁；结果见 [CBI-01](cbi_01_source_import_plan.md)。CBI-02 已完成持续 D3D12 接入及视觉/回归验收；CBI-03 同代证据已完成，CPU工程性能对照保留回归待查；CBI-04 尚未验收。
...
|---|---|---|---|
| **CBI-01 来源冻结与模块迁入** | 记录源文件/ABI/依赖/版权；迁入核心、着色器和测试；公共声明、编译门禁与构建接通；尚不默认启用 GUI CBT | 七组来源核心测试按职责覆盖；四容量着色器可编译；OpenGL/CPU-only 不误链 D3D12。修改前后同机短 CPU 基线；核心有限测试旧/新成本和构建成本分别记录 | 已完成 |
| **CBI-02 D3D12 持续接入** | 设备能力、描述符、工厂、GPU 输出、间接绘制、GUI/CLI及生命周期；保留当前材质 | test129 短序列与 Peking 既有返回轨迹；实际看填充/线框、暂停/单步/Reset/切换；来源与迁入版本同条件迁移对照；CPU D3D12/GL 受影响路径前后快验 | 已完成 |
| **CBI-03 实验网格与计量接入** | 同代实际 GPU 网格捕获；case/CSV协议、容量校验、独立质量与视觉证据 | 小网格 CPU 几何 oracle 与实际 GPU 位置核对；Peking 关键帧的截图/矩阵/活动 N/mesh 对齐；关闭捕获前后性能；开启捕获的读回成本单列 | 证据已完成；CPU性能待查 |
| **CBI-04 首轮有限质量比较与收口** | 冻结小面板与 CBT 参数点，复用 EIP 生成质量—N图、轨迹/见证图与分边界时间表；沉淀命令/限制 | 同后端同输入重新取得必要 CPU 对照；真实画面验收；不以 CBT 或 Transactional 胜负为接入门禁。报告生成改动核对前后成本 | 有限面板完成；51/56质量有效 |
...
- CBI-01：已完成，提交 8631320。
- CBI-02：平台接入验收已完成，质量捕获另归 CBI-03。
- CBI-03：同代证据已完成；CPU工程性能OPEN，详见小规划。
- CBI-04：14配置/28进程/56关键评价已完成，八张图已实际查看；51个质量评价有效，5个canyon CBT点因覆盖歧义无效。详见[小规划](cbi_04_first_comparison_plan.md)和[报告](../../research/cbt_2024/cbi_04_comparison_results.md)。
- CBI-01 开始前已完成源码边界核对；阶段结果分别回填，后续未完成项不预先标记通过。

### 接入收口的分类状态

- 模块迁入、D3D12持续接入、同代实际GPU捕获：已有有限通过证据。
- 首轮有限比较和视觉报告：完成，不以算法胜负作为接入门槛。
- Canyon部分CBT采样覆盖：5点失败，根因待查；不以无效指标作优势比较。
- CBI-03 CPU工程性能：OPEN。本轮CPU使用保留的CBI-02程序，不同二进制身份和限制已写入报告。
- 持续质量普遍优势、同质量性能与论文竞争性：未证明。本轮不继续扩大GPU算法、参数矩阵或性能优化范围。
