...
实施结果：2026-09-09 完成五个文件的注释优化。当时将五处 summary 压缩为包含标签的三行，但“过长”的判断错误，现按第 8.4 节撤回；八行禁止的行尾标点已清理，同时去掉两处历史阶段编号、`C1` 标签与“新下限”时序表述。数量适配注释明确零工作返回 0；补充脏槽位工作量、同步借用和无线程池时顺序执行的限制，完整网格重写说明包含全量上传请求。
...
用户要求逐个读取阶段文件，手工将注释改为自然且符合实际语义的说明，不使用脚本批量替换。三行限制只计算正文，不含 `summary` 开闭标签；第 8.3 节将标签计入行数并压缩正文的做法不再作为合规依据。

用户进一步澄清重点是句内标点和语义，并非把所有摘要改成两行。清楚的一行说明保留；只有需要分别交代职责和约束时才分行，不以行数作为改写目标。

本轮沿第 4 节的实际实现文件逐一检查声明、实现及相关调用。全部采用 Extend：公共策略与输入哈希在算法公共头文件内解释请求和身份边界；Parallel、Queues、MeshEmit 分别解释数量解析、评分与网格写入；Benchmark 选项、解析和运行文件保留各自输入与配置职责；共享 CSV、测试及构建文件逐个核对，已有清楚的说明保留。只在现有文件中手工修改注释，不改变接口、职责或依赖。

完成后逐项审阅实际差异，核对注释与条件、返回值、所有权和执行顺序一致，并检查正文行数、标点、覆盖率及无可执行代码变化；结果写回本节与注释专项审查。此前 PREP-03 的未提交注释修改保留，本轮不提交 Git。

实施结果：17 个实现、测试及构建文件已逐个读完，手工修改其中 10 个源码文件的注释，其余文件保留。重点修正请求与实际任务数量、配置哈希的能力边界、资源借用检查、场景目的及计时含义；清楚的一行注释没有统一展开。与 PREP-02 和此前 PREP-03 修改合并核查后，当前 27 个修改的 C++ 文件去除整行注释均与 `570ecd0` 一致，99 个摘要的正文均未超过三行，标签和行尾标点符合规范。覆盖率为 `src` 2722/17178 = 15.8%、DOD 1032/5002 = 20.6%，其余模块也满足门槛。未改变架构或执行行为，未重复功能测试，未暂存或提交；逐文件记录见[专项审查第 8 节](../../reviews/formal_experiment/prep_01_02_comment_compliance_review.md#8-逐文件阅读与语义复核)。
