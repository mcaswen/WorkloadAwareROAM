...
| 兼容 | 原 24 项 CTest 和新测试均注册运行；两个后端共享 CPU 实现，公共注释使用三行 summary |
...
验收按上述问题逐项核对注释准确性，检查三行 summary、连续注释长度、行尾标点和阶段编号；对修改前快照确认源码差异仅为整行注释，再运行注释覆盖率门禁。纯注释变更无需重复两后端功能测试。整改结果完成后同步本节与两份相关审查。
