...
**实现情况**：2026-09-11 已进入规划，形成 [P5 小规划](prep_05_cpu_crossover_pilot_gate_plan.md)，尚未采集 P5 判断数据。提案只复用已有组件和 113 个目标，以 OpenGL 三次 W=3/R=12/P=8 完整集合进行探索；严格结果与单次噪声门槛保持，三次配对方向全部一致才支持目标优势，旧 P4 的 5% 动作漂移明确保留为风险。该事前取舍待 Review，不通过遗漏旧短运行数据宣称环境合格。执行后记录数据目录、覆盖、噪声、逐阶段结论、是否补测，以及人类 Review 的继续/收缩/停止决定；结果不支持继续时，可靠记录与负面报告仍构成本阶段的完整交付。
