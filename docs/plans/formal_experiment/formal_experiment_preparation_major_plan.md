...
> 当前处置（2026-09-12）：PREP-05 已[完成探索与审查](../../reviews/formal_experiment/prep_05_cpu_crossover_pilot_gate_review.md)，原完整路线门槛未满足。当前研究改以[CPU 多阶段执行、并行优化与性能边界](cpu_roam_research_definition.md)组织；本页下方状态及继续条件保留为历史记录，PREP-06～10 不自动恢复。

> 规划类型：大规划（Major Plan）\
> 状态：PREP-01～PREP-03 已完成；PREP-04 核心实现为 `7522db7`、记录为 `a738f78`；原性能问题暂时关闭，连续前缀已实现并完成新全集单次配对，新增规划成本待用户确认承接；PREP-04 尚未整体验收，PREP-05～PREP-10 未开始\
...
| AC-12 | 每个小阶段保存性能基线、同条件复测与逐项比对；按开发规范第 7.3 节判断有实际影响的退化，由用户决定修复路径或明确暂时关闭，保留证据与处置边界，具体要求见第 12.4 节 |
...
**最新移交决定（2026-09-11）**：用户要求最后排查后写明限制进入 P5。现有 60 次短运行已独立重算，未发现记录或计算错误；不再追加采样，保留 20/22 个动作组合漂移越线的事实。PREP-04 以“工程交付结束，环境限制保留”移交 PREP-05，不追补旧环境验收结论，也不改写旧数据排除结果。源码提交为 `ec8792e`，具体移交依据见[最后审查](../../reviews/formal_experiment/prep_04_timing_stability_final_review.md)。下段无条件验收未通过仍是技术事实，继续 P5 的流程阻断已由用户此次授权解除。

**当前状态（2026-09-11 收尾）**：实现及规定运行已完成，严格结果等价通过，性能问题按用户决定关闭。当前两后端各 113 目标全集、10 个固定首目标的名义重复和两轮独立短运行共 72 个应用进程全部核验完成。短运行唯一复测后，22 个目标/动作组合仍有 20 个超过原定 5% 漂移上限，已停止采样并排除相关来源的胜负解释；采样环境验收未通过，因此不标记 PREP-04 整体通过或自动进入 PREP-05。自然细分拓扑实际并行覆盖仍为 0/24 个所选目标，受控正例不补充研究样本。详细证据见[小规划第 11.9 节](prep_04_minimal_cpu_pairing_plan.md#119-性能关闭后的剩余收尾)和[当前审查](../../reviews/formal_experiment/prep_04_minimal_cpu_pairing_architecture_review.md#6-2026-09-11-剩余运行收尾审查)；下方实现与修复段落按时间保留历史状态。
...
**实现情况**：小规划已获用户确认并实施，包含 Measurement/Evidence 拆分、真实目标重建、配对 v2、标定、实际进程追溯和最小分析。两后端完整构建及各 36 项 CTest 完成，最终 Python 审计与容量复测也完成。六场景完整集合发现既有细分拓扑策略不等价；普通矩阵两轮出现性能恶化。用户随后分别新增[拓扑等价性修复小规划](prep_04_split_topology_equivalence_fix_plan.md)和[性能修复小规划](prep_04_runtime_performance_fix_plan.md)，阶段编号不变。性能方案以 Minor Revision 批准后，已补三项实验契约、实现测量工具与 Probe，并完成 1,488 次有效性能进程及原 25 项逐项复核；部分局部差值未满足关闭契约，未形成具体生产修复，整体性能验收仍受阻。拓扑修复方案仍待 Review。详见[小规划实现记录](prep_04_minimal_cpu_pairing_plan.md#11-实现情况与收尾记录)、[架构审查](../../reviews/formal_experiment/prep_04_minimal_cpu_pairing_architecture_review.md)、[等价性分析](../../reviews/formal_experiment/prep_04_split_topology_equivalence_analysis.md)及[性能调查](../../reviews/formal_experiment/prep_04_runtime_performance_regression_analysis.md)。

**2026-09-10 承接更新**：上段为调查结束时的技术状态。用户随后将普通运行性能问题暂时关闭，并授权先提交测量工作、再实施拓扑连续前缀修复；测量代码提交为 `ea90d2e`。该决定解除原性能问题的阻断，拓扑方案进入实施，不改变结果等价和研究判断标准。后续工程性能门槛见开发规范第 7.3 节。

**2026-09-11 拓扑进展**：性能规范与处置记录另提交为 `662a730`。连续前缀方案 A 已实现，统一串行优先级并保留真实停止项、优先合并与预算边界；两后端完整构建及 40/40 CTest 通过，第 14 帧原反例与旧 113 个核心输入等价回归通过。新发现确认六场景 384 个细分前缀全部为空，受控两块正例验证真实并行能力，但不能补充研究覆盖。新 OpenGL 全集 W=1/R=6/P=8 的 113 个目标完成严格配对及分析核验。普通性能两轮共 228 个进程，标准场景新增约 0.10 ms 规划成本稳定越线，[独立分析](../../reviews/formal_experiment/prep_04_split_prefix_planning_cost_analysis.md)已给出原因边界和承接选项，等待用户决定；原性能问题仍暂时关闭。剩余独立全集、跨后端全集、名义重复与短运行尚未完成，详见[拓扑小规划第 9 节](prep_04_split_topology_equivalence_fix_plan.md#9-实现情况)及[专门架构审查](../../reviews/formal_experiment/prep_04_split_topology_equivalence_architecture_review.md)；本轮没有提前作出 CPU gate 投入决定。

**2026-09-11 空前缀成本修复**：用户批准[独立短路小规划](prep_04_split_prefix_planning_cost_fix_plan.md)，已省去必为空时的真实队列扫描，完整计划与结果保持。按新增规范只跑三个相关测试，均一次通过；性能选择两个 OpenGL 默认场景，首轮 36 个进程，仅标准场景复测一次 18 个进程后停止。标准规划中位数由约 0.122 ms 降至 0.029 ms，相对问题前的配对差约 +0.005 ms；但规划 P95 相对问题前仍稳定增加约 0.096 ms，剩余非空前缀成本未解决。所选指标无短路新增越线；本问题只记为部分缓解，不宣告 PREP-04 整体性能验收完成。分析、证据与[增量架构审查](../../reviews/formal_experiment/prep_04_split_prefix_planning_cost_fix_architecture_review.md)已同步，未擅自扩展非空路径修复或继续采样。此前“等待用户决定承接”指短路实施前，当前待决的是剩余成本的处理。

**2026-09-11 当前决定**：用户明确关闭上述剩余性能问题，保留原始差值及分析，不再要求处理同一 P95 成本，解除对应开发阻断。已知拓扑不等价反例已修复，原输入与新目标配对的已测范围通过，当前没有新反例；自然非空前缀覆盖不足和 PREP-04 剩余独立运行仍分别跟踪，不将性能关闭视为 PREP-04 整体验收。当前状态见[拓扑小规划第 9.7 节](prep_04_split_topology_equivalence_fix_plan.md#97-性能关闭后的拓扑状态)，关闭依据见[性能分析第 6 节](../../reviews/formal_experiment/prep_04_split_prefix_planning_cost_analysis.md#6-用户关闭决定)。

### PREP-05：CPU crossover pilot gate

**依赖**：PREP-01、PREP-02、PREP-03、PREP-04 通过可靠性验收；执行前完成 [P5 小规划](prep_05_cpu_crossover_pilot_gate_plan.md) Review，固定场景、目标选择、噪声规则、重复规模和一次补测上限。该 gate 只依赖 CPU 能力。

**本次移交例外**：2026-09-11 用户允许 P4 最后排查仍不能解决漂移时，写明限制进入 P5。严格结果等价、输入与记录完整性继续作为硬门槛；不要求重新采集 P4 的动作耗时 5% 验收。P5 必须在自己的小规划中先明确探索判断规则及旧环境风险的处理，再采集判断数据；本次例外不放宽正式研究门槛，也不授权绕过 CPU gate 进入 PREP-06。
...
**实现情况**：2026-09-11 用户确认 [P5 小规划](prep_05_cpu_crossover_pilot_gate_plan.md)后完成闭环。OpenGL 三次 W=3/R=12/P=8 全集均通过 113 个目标的严格等价与来源核验，共 9,000 条计量、2,250 条预热及 12,000 条标定；正式源码和脚本未修改。三次配对方向契约在采样前获准，P4 的环境风险继续保留，本轮也不声称正式环境合格。

**投入建议：收缩研究范围。** 只有网格串行脏写/并行脏写满足双向信号要求，串行侧 4 个、并行侧 3 个输入三次均超过阈值；绝对工作量可初步描述两侧，但地形与规模尚未分离。合并评分为串行 1 / 并行 17 个稳定点，串行侧不足；细分评分仅并行侧 19 个；合并拓扑实际并行 5/17，细分拓扑 0/24，均未形成合格双向证据。当前不满足至少三个阶段的完整路线条件，建议保留网格方向、暂停通用路线和上传投入；最终范围待用户 Review，未擅自改写后续实施方案。

没有追加补测、重跑普通性能矩阵或追查已关闭问题。P5 的采集、分析、特征核对及审查交付完成，详细数据、限制和成本见[小规划第 9 节](prep_05_cpu_crossover_pilot_gate_plan.md#9-实施与闭环记录)及[投入审查](../../reviews/formal_experiment/prep_05_cpu_crossover_pilot_gate_review.md)。

### PREP-06：正式输入、运行时与上传基础

**当前入口状态**：PREP-05 仅一个 CPU 阶段满足双向信号规则，已提出收缩建议；当前不启动本阶段。下列完整路线设计保留为原规划，待用户确认研究范围后再决定是否承接或调整，不假设上传能够补足 CPU gate 的证据。
...
2026-09-11 起按用户要求采用[开发规范第 5.8 节](../../standards/development_guidelines.md#58-按影响选择测试)：每次修改由 Agent 自主选择足够的相关测试，不默认重跑全量 CTest、两个后端或所有历史矩阵。下列分层是覆盖归属，不是每次修改都要执行的清单；已有有效结果可复用，新增风险才扩大验证。研究协议要求的独立运行另行保留，不能误作冗余工程测试删除。
...
性能对照同样按影响选择代表场景和指标；“逐项比对”指小规划冻结的相关子集。局部修复可复用已有归档程序、源码及报告作为基线，不重新采集全部矩阵。只修改共享 CPU 逻辑时可选一个后端，涉及上传或后端差异时才分别覆盖；具体选择、复用理由和结论范围必须记录。

**当前工程口径：** 2026-09-10 起以[开发规范第 7.3 节](../../standards/development_guidelines.md#73-工程性能回归的实际影响门槛)的实际影响门槛及一次复测上限为准。几微秒局部差值只记录，不持续阻断。下列“修复后关闭”指技术修复型关闭；用户也可明确暂时关闭已知问题，保留未决原因并指定继续范围，不重写历史报告或研究阈值。
