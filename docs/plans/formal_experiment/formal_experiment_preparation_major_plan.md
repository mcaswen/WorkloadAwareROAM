...
AC-11 贯穿全部阶段。原先延后的能力现在各有明确归属：完整场景与图形输入由 PREP-06 交付，正式记录契约由 PREP-07 冻结，完整统计由 PREP-08 交付，文件身份/存储及 CPU 正式运行集成由 PREP-09 交付；PREP-02、PREP-04 在各自最小范围验收后即可完成，无需等待后续阶段回填。
...
5. 每阶段小规划及本大规划的“实现情况”记录实际提交、测试、输出位置和审查结果；未实施项保持“未开始”，不能因文档写完改为“已完成”。
