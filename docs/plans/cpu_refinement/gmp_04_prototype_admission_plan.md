...
准入条件逐项映射：直接需求来源、条件式事务正确性、自然批次、费用边界、续接义务和已有工作差异。结论允许为“受限实验原型可继续，生产/长期质量不通过”，不要求伪造无保留 Pass。若已有工作覆盖全部候选执行差异，或质量问题使可声明的实验契约本身不一致，则不进入实现，记录具体缺口。
...
决策为受限持续原型可进入设计 Review；生产/持续质量及论文新颖性不通过或尚未验证，不给无保留算法成功结论。下一规划先实现直接串行单批，再局部续接/高度保护，再动态参考和短轨迹，最后有限多核；v1 与保护变体分开，保护结果未测。Adams 等先例的关键重合已经记录，文献正文不可读的部分明确降级为摘要范围。
