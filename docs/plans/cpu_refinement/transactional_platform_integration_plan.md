...
- 实现情况：TPI-02 公共适配、一次种子、raw 版本、NO/ZO、借用与失败恢复完成，见[小规划](tpi_02_public_adapter_plan.md)；图形和持续质量仍待后续阶段。
...
- 实现情况：TPI-03 开关、持续调度、独立统计和原生 OpenGL 生命周期检查完成，见[小规划](tpi_03_opengl_application_plan.md)；默认短回放零事务，性能和持续质量不据此通过。
...
- 实现情况：完成原生 D3D12 接入、帧轮转和资源恢复；见[小规划](tpi_04_d3d12_consumption_plan.md)。自然非空交易覆盖留到 TPI-05，不以零事务暖帧宣称性能通过。
