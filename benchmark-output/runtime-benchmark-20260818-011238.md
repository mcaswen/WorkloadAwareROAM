...
- 详细 CSV：`runtime-benchmark-20260818-011238.csv`
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 64 | 219.62 | 1375.84 | 210.91 | 658.21 | 200000.00 | 200000 | 1125600.03 | 1614326 | 99.16 | 106.40 | 1 | 20 | 20 | 0 |
| Data-Oriented CPU ROAM | 64 | 131.01 | 699.10 | 121.00 | 399.00 | 199999.55 | 200000 | 1125597.62 | 1614322 | 154.97 | 418.19 | 8 | 20 | 20 | 0 |
| GPU ROAM-like | 64 | 199.95 | 540.07 | 193.40 | 403.36 | 199999.50 | 200000 | 1125251.69 | 1613570 | 122.07 | 195.50 | 8 | 20 | 20 | 0 |
...
| --- | ---: | ---: | ---: | ---: | --- |
| Prepare / 帧状态 | 7.6217 | 8.2695 | 8.6561 | N/A | CPU 准备持久拓扑和 snapshot 输入 |
| Merge 候选评分 | 14.1750 | 2.4940 | 2.5042 | 1.0691 | Shader 为诊断重新扫描已 split 的父节点；GPU 不提交这些候选 |
| Merge 拓扑提交 / 向上级联 | 18.5305 | 19.7974 | 20.3305 | N/A | 持久 merge、邻接修复和级联回收由 Classic/DOD CPU 路径执行；GPU shader 未提交 merge |
| Split 前 active leaf 收集 | 0.0000 | 0.0000 | 0.0000 | 1.0367 | DOD CPU 直接以持久 Q_s 表示 active leaf 与预算计数，本列为 0；GPU 在评分前仍单独压缩 snapshot |
| 视点相关 leaf error / 视锥测试 | 0.0000 | 0.0000 | 0.0000 | 0.1814 | Classic 与 DOD 都把评分计入持久 Q_s 优先级刷新，本列为 0；GPU 基于 snapshot 单独评分 |
| Split 扫描/标记 | 41.7716 | 5.7385 | 7.1629 | 0.0783 | DOD CPU 数值包含持久 Q_s 的并行 SSE/视锥优先级刷新、原地建堆和提交快照生成；GPU append 顺序不是误差优先级顺序 |
| Split 拓扑 / 裂缝约束提交 | 35.9098 | 31.0255 | 61.0459 | 0.1662 | GPU 只提交一轮受预算约束的细分，并只处理直接 base-neighbor diamond pair；没有递归 forced-split 链 |
| 细分后 active leaf 收集 / 输出视图 | 0.0000 | 0.0000 | 0.0000 | 0.8964 | Classic 直接复用 dense mesh slot owners，DOD 直接复用 ActiveLeafNodes，因此两者本列均为 0；GPU 数值包含 counter reset 和 split 后 leaf compaction |
| Mesh emit / draw argument 生成 | 27.1664 | 8.7189 | 0.0000 | 0.3700 | Classic 只重写拓扑变化影响的稳定槽位；DOD 仍完整 emit；GPU 输出非共享顶点、索引和 indirect draw argument |
| Finalize / 发布 packet | 53.6932 | 28.9509 | 28.4002 | N/A | CPU 发布统计和 renderer 资源契约 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 198.8699 | 7.6217 | 14.1750 | 18.5305 | 0.0000 | 0.0000 | 41.7716 | 35.9098 | 0.0000 | 27.1664 | 53.6932 | 11.8822 |
| Data-Oriented CPU ROAM | 104.9953 | 8.2695 | 2.4940 | 19.7974 | 0.0000 | 0.0000 | 5.7385 | 31.0255 | 0.0000 | 8.7189 | 28.9509 | 15.7601 |
| GPU ROAM-like | 115.3462 | 8.6561 | 2.5042 | 20.3305 | 0.0000 | 0.0000 | 7.1629 | 61.0459 | 0.0000 | 0.0000 | 28.4002 | 21.2369 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic Split | 0 | 0 | 0 | 0 | 0 | 35.9098 | 35.9098 | 35.9098 |
| DoD Split | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 31.0249 | 31.0249 | 31.0255 |
| Merge | 0.1899 | 0.9307 | 0.3365 | 0.0079 | 3.5995 | 14.7227 | 19.7872 | 19.7974 |
...
| --- | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 1 | 67501.1562 | 132498.8438 | 24442.5000 | 33600000 |
| Data-Oriented CPU ROAM | 1 | 67500.1719 | 132499.3750 | 35292.5625 | 33600000 |
| GPU ROAM-like | 0 | 0.0000 | 0.0000 | 0.0000 | 180719892 |
...
| --- | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 77.6814 | 32.7055 | 27.1664 | 0.0000 |
| Data-Oriented CPU ROAM | 48.9494 | 22.2913 | 8.7189 | 0.0000 |
| GPU ROAM-like | 68.2088 | 22.8347 | 0.0000 | 0.0000 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| GPU ROAM-like | 1.0367 | 0.1814 | 0.0783 | 1.0691 | 0.1662 | 0.0062 | 0.8902 | 0.3700 | 3.7981 | 10.7817 |
...
| GPU ROAM-like | 30.34 | 4.77 | 0.29 | 0.00 | 18.96 | 0.00 | 0.00 | 0.00 | 180719892 | 96 |
