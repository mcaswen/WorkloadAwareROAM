...
- 详细 CSV：`runtime-benchmark-20260831-035824.csv`
- CPU 上传配对 CSV：`runtime-benchmark-20260831-035824-cpu-upload.csv`
...
- Benchmark 标签：phase5-upload-d3d12-final
- 算法顺序：Classic CPU ROAM -> Data-Oriented CPU ROAM；轮换偏移 0
- 独立预热：每种算法 0 个不计入结果的采样点，预热后重置拓扑再开始记录
- CPU 上传配对：每个数据包预热 0 次，正式重复 1 次，目标数量 1
- Benchmark 路径：默认选项路径
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 4 | 23.68 | 46.47 | 24.65 | 36.95 | 10660.25 | 12591 | 35647.50 | 45676 | 98.44 | 125.07 | 1 | 14 | 14 | 0 |
| Data-Oriented CPU ROAM | 4 | 25.06 | 36.79 | 18.94 | 24.63 | 10660.25 | 12591 | 35647.50 | 45676 | 103.53 | 127.48 | 8 | 14 | 14 | 0 |
...
| --- | ---: | ---: | --- |
| Prepare / 帧状态 | 0.4744 | 0.4517 | 准备持久拓扑和逐帧输入 |
| Merge 候选评分 | 0.3135 | 0.1336 | 刷新持久 Q_m 的优先级 |
| Merge 拓扑提交 / 向上级联 | 3.0530 | 4.5196 | 提交 diamond merge、邻接修复和级联回收 |
| Split 前 active leaf 收集 | 0.0000 | 0.0000 | 两者均直接复用持久活动叶表示 |
| 视点相关 leaf error / 视锥测试 | 0.0000 | 0.0000 | 计入持久 Q_s 优先级刷新，因此独立字段为 0 |
| Split 扫描/评分 | 0.5047 | 0.1619 | 刷新 Q_s 分数并原地建堆 |
| Split 拓扑 / 裂缝约束提交 | 9.5442 | 7.7207 | 按同一预算与 forced-split 约束提交 |
| 细分后 active leaf 收集 / 输出视图 | 0.0000 | 0.0000 | Classic 复用 dense slot owners，DOD 复用 ActiveLeafNodes |
| Mesh emit / draw argument 生成 | 4.2872 | 1.2728 | 两者均只重写 dirty slot；DOD 可将较大批次分段给 worker |
| Finalize / 发布 packet | 0.8777 | 0.6240 | 汇总统计、更新跨帧状态并发布 renderer packet |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 19.0555 | 0.4744 | 0.3135 | 3.0530 | 0.0000 | 0.0000 | 0.5047 | 9.5442 | 0.0000 | 4.2872 | 0.8777 | 0.8331 |
| Data-Oriented CPU ROAM | 14.8844 | 0.4517 | 0.1336 | 4.5196 | 0.0000 | 0.0000 | 0.1619 | 7.7207 | 0.0000 | 1.2728 | 0.6240 | 0.0965 |
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | Q_m | 1672.0000 | 0.7500 | 0.2943 | 0.0192 | 0.3135 | 33219.2500 | 2.0665 | 0.0000 |
| Classic CPU ROAM | Q_s | 8525.0000 | 1.0000 | 0.4260 | 0.0787 | 0.5047 | 27937.2500 | 2.1286 | 0.0000 |
| Data-Oriented CPU ROAM | Q_m | 1672.0000 | 6.0000 | 0.1169 | 0.0167 | 0.1336 | 31635.2500 | 1.8530 | 0.0059 |
| Data-Oriented CPU ROAM | Q_s | 4936.5000 | 6.2500 | 0.1370 | 0.0249 | 0.1619 | 27936.7500 | 1.9429 | 0.0000 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic Split | 0 | 0 | 0 | 0 | 0 | 9.5442 | 9.5442 | 9.5442 |
| DoD Split | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 7.7191 | 7.7191 | 7.7207 |
| Merge | 0.0612 | 0.5246 | 0.1278 | 0.0053 | 2.1753 | 1.5727 | 4.4669 | 4.5196 |
...
| Data-Oriented CPU ROAM | 1 | 10479.0000 | 181.2500 | 163.5000 | 2115288 |
...
| --- | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 10.0489 | 3.3666 | 4.2872 | 0.0000 |
| Data-Oriented CPU ROAM | 7.8826 | 4.6533 | 1.2728 | 0.0000 |
