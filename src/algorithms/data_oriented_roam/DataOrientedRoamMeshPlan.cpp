...
namespace
{
void MarkMeshSlotDirty(DataOrientedRoamMeshMetadata& mesh, std::size_t slot)
{
...
void NormalizeDirtyMeshSlots(DataOrientedRoamMeshMetadata& mesh)
{
...
void ResizeMeshForSlotCount(DataOrientedRoamMeshMetadata& mesh)
{
    mesh.SlotDirtyGenerations.resize(mesh.SlotOwners.size(), 0U);
}

void InitializeIncrementalMesh(const DataOrientedRoamMeshPlanInput& state, DataOrientedRoamMeshMetadata& mesh)
{
...
bool AppendMeshLeaf(const DataOrientedRoamMeshPlanInput& state, DataOrientedRoamMeshMetadata& mesh, DataOrientedRoamNodeIndex node)
{
...
bool RemoveMeshLeaf(const DataOrientedRoamMeshPlanInput& state, DataOrientedRoamMeshMetadata& mesh, DataOrientedRoamNodeIndex node)
{
    if (!state.IsValidNode(node) || node >= mesh.NodeSlots.size())
    {
        return false;
    ...
    if (removedSlot != lastSlot)
    {
        ...
        MarkMeshSlotDirty(mesh, removedSlot);
    }
...
bool ReplaceMeshLeafWithChildren(const DataOrientedRoamMeshPlanInput& state, DataOrientedRoamMeshMetadata& mesh, DataOrientedRoamNodeIndex parent)
{
    ...
    MarkMeshSlotDirty(mesh, parentSlot);
    return AppendMeshLeaf(state, mesh, rightChild);
}

bool ReplaceMeshChildrenWithLeaf(const DataOrientedRoamMeshPlanInput& state, DataOrientedRoamMeshMetadata& mesh, DataOrientedRoamNodeIndex parent)
{
    if (!state.IsValidNode(parent) || parent >= mesh.NodeSlots.size())
    {
        return false;
    ...
    MarkMeshSlotDirty(mesh, parentSlot);
    return RemoveMeshLeaf(state, mesh, removedChild);
}

}

bool DataOrientedRoamMeshPlanInput::IsValidNode(DataOrientedRoamNodeIndex node) const
{
    return node < Nodes.size();
}

bool DataOrientedRoamMeshPlanInput::IsLeaf(DataOrientedRoamNodeIndex node) const
{
    return IsValidNode(node) && Nodes.IsSplitAt(node) == 0U;
}

void BeginDataOrientedRoamMeshPlan(
...
bool ApplyDataOrientedRoamMeshPlan(
    const DataOrientedRoamMeshPlanInput& state, DataOrientedRoamMeshMetadata& mesh)
{
    bool reinitialized = mesh.NeedsInitialization;
    if (mesh.NeedsInitialization)
    {
        InitializeIncrementalMesh(state, mesh);
        reinitialized = true;
    }
    else
    {
        ...
        for (const DataOrientedRoamMeshTopologyEdit& edit : mesh.TopologyEdits)
        {
            replaySucceeded = edit.Type == DataOrientedRoamMeshTopologyEditType::Split
                ? ReplaceMeshLeafWithChildren(state, mesh, edit.Node)
                : ReplaceMeshChildrenWithLeaf(state, mesh, edit.Node);
            if (!replaySucceeded)
            {
                break;
            }
